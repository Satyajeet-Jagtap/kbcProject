
//     {
//     id:1,
//     question:"Rolex is a company that specializes in what type of product?",
//     answer:[
//       {
//       text:"Phone",
//       correct:false,
//     },
//     {
//       text:"Watches",
//       correct:true,
//     },
//     {
//       text:"Food",
//       correct:false,
//     },
//     {
//       text:"Cosmetic",
//       correct:false,
//     }]
//   },
//   {
//     id:2,
//     question:"Current Railway Minister of India is",
//     answer:[
//       {
//       text:"Mamta Banarjee",
//       correct:false,
//     },
//     {
//       text:"Ram Vilash",
//       correct:false,
//     },
//     {
//       text:"Ashwini Vaishnaw",
//       correct:true,
//     },
//     {
//       text:"Piyush Goyal",
//       correct:false,
//     }]
//   },
//   {
//     id: 3,
//     question: "What is the capital of France?",
//     answer: [
//       { text: "Paris", correct: true },
//       { text: "London", correct: false },
//       { text: "Berlin", correct: false },
//       { text: "Madrid", correct: false }
//     ]
//   },
//   {
//     id: 4,
//     question: "Who is the founder of Microsoft Corporation?",
//     answer: [
//       { text: "Bill Gates", correct: true },
//       { text: "Steve Jobs", correct: false },
//       { text: "Mark Zuckerberg", correct: false },
//       { text: "Larry Page", correct: false }
//     ]
//   },
//   {
//     id: 5,
//     question: "Which planet is known as the Red Planet?",
//     answer: [
//       { text: "Mars", correct: true },
//       { text: "Venus", correct: false },
//       { text: "Jupiter", correct: false },
//       { text: "Saturn", correct: false }
//     ]
//   },
//   {
//     id: 6,
//     question: "What is the chemical symbol for water?",
//     answer: [
//       { text: "H2O", correct: true },
//       { text: "CO2", correct: false },
//       { text: "NaCl", correct: false },
//       { text: "O2", correct: false }
//     ]
//   },
//   {
//     id: 7,
//     question: "Who is the founder of Microsoft Corporation?",
//     answer: [
//       { text: "Bill Gates", correct: true },
//       { text: "Steve Jobs", correct: false },
//       { text: "Mark Zuckerberg", correct: false },
//       { text: "Larry Page", correct: false }
//     ]
//   },
//   {
//     id: 8,
//     question: "Which planet is known as the Red Planet?",
//     answer: [
//       { text: "Mars", correct: true },
//       { text: "Venus", correct: false },
//       { text: "Jupiter", correct: false },
//       { text: "Saturn", correct: false }
//     ]
//   },
//   {
//     id: 9,
//     question: "What is the chemical symbol for water?",
//     answer: [
//       { text: "H2O", correct: true },
//       { text: "CO2", correct: false },
//       { text: "NaCl", correct: false },
//       { text: "O2", correct: false }
//     ]
//   },
//   {
//     id:10,
//     question:"Current Railway Minister of India is",
//     answer:[
//       {
//       text:"Mamta Banarjee",
//       correct:false,
//     },
//     {
//       text:"Ram Vilash",
//       correct:false,
//     },
//     {
//       text:"Ashwini Vaishnaw",
//       correct:true,
//     },
//     {
//       text:"Piyush Goyal",
//       correct:false,
//     }]
//   },
//   {
//     id:11,
//     question: "What is the capital of France?",
//     answer: [
//       { text: "Paris", correct: true },
//       { text: "London", correct: false },
//       { text: "Berlin", correct: false },
//       { text: "Madrid", correct: false }
//     ]
//   },
//   {
//     id:12,
//     question: "Who is the founder of Microsoft Corporation?",
//     answer: [
//       { text: "Bill Gates", correct: true },
//       { text: "Steve Jobs", correct: false },
//       { text: "Mark Zuckerberg", correct: false },
//       { text: "Larry Page", correct: false }
//     ]
//   }
//   ,
//   {
//     id: 13,
//     question: "What is the chemical symbol for water?",
//     answer: [
//       { text: "H2O", correct: true },
//       { text: "CO2", correct: false },
//       { text: "NaCl", correct: false },
//       { text: "O2", correct: false }
//     ]
//   },
//   {
//     id:14,
//     question:"Current Railway Minister of India is",
//     answer:[
//       {
//       text:"Mamta Banarjee",
//       correct:false,
//     },
//     {
//       text:"Ram Vilash",
//       correct:false,
//     },
//     {
//       text:"Ashwini Vaishnaw",
//       correct:true,
//     },
//     {
//       text:"Piyush Goyal",
//       correct:false,
//     }]
//   },
//   {
//     id:15,
//     question: "What is the capital of France?",
//     answer: [
//       { text: "Paris", correct: true },
//       { text: "London", correct: false },
//       { text: "Berlin", correct: false },
//       { text: "Madrid", correct: false }
//     ]
//   },



  // Easy Level (1-5)
    const data=[
    {
      id: 1,
      question: "Which of the following is used to style web pages?",
      answer: [
        { text: "CSS", correct: true },
        { text: "HTML", correct: false },
        { text: "JavaScript", correct: false },
        { text: "SQL", correct: false },
      ],
    },
    {
      id: 2,
      question: "Which of the following is NOT a programming language?",
      answer: [
        { text: "Python", correct: false },
        { text: "Ruby", correct: false },
        { text: "HTML", correct: true },
        { text: "Java", correct: false },
      ],
    },
    {
      id: 3,
      question: "What does the 'console.log()' function do in JavaScript?",
      answer: [
        { text: "Outputs messages to the browser's console", correct: true },
        { text: "Writes text to a file", correct: false },
        { text: "Runs an HTTP request", correct: false },
        { text: "Creates an alert box", correct: false },
      ],
    },
    {
      id: 4,
      question: "What is the purpose of a loop in programming?",
      answer: [
        { text: "To repeat a block of code", correct: true },
        { text: "To terminate a program", correct: false },
        { text: "To create a variable", correct: false },
        { text: "To display a message", correct: false },
      ],
    },
    {
      id: 5,
      question: "Which of the following is a database management system?",
      answer: [
        { text: "MySQL", correct: true },
        { text: "React", correct: false },
        { text: "Node.js", correct: false },
        { text: "CSS", correct: false },
      ],
    },

    // Medium Level (6-10)
    {
      id: 6,
      question: "What is the result of '3 + 2 + \"7\"' in JavaScript?",
      answer: [
        { text: "57", correct: true },
        { text: "12", correct: false },
        { text: "NaN", correct: false },
        { text: "37", correct: false },
      ],
    },
    {
      id: 7,
      question: "Which HTTP method is used to update a resource?",
      answer: [
        { text: "PUT", correct: true },
        { text: "GET", correct: false },
        { text: "POST", correct: false },
        { text: "DELETE", correct: false },
      ],
    },
    {
      id: 8,
      question: "Which is a commonly used JavaScript framework for building front-end applications?",
      answer: [
        { text: "React", correct: true },
        { text: "Laravel", correct: false },
        { text: "Spring Boot", correct: false },
        { text: "Django", correct: false },
      ],
    },
    {
      id: 9,
      question: "What is the purpose of the 'async' keyword in JavaScript?",
      answer: [
        { text: "To allow a function to return a promise", correct: true },
        { text: "To declare a synchronous function", correct: false },
        { text: "To pause execution of a loop", correct: false },
        { text: "To execute a function immediately", correct: false },
      ],
    },
    {
      id: 10,
      question: "What is a middleware in Express.js?",
      answer: [
        { text: "A function that executes before the final request handler", correct: true },
        { text: "A database query handler", correct: false },
        { text: "A front-end component", correct: false },
        { text: "A routing library", correct: false },
      ],
    },

    // Hard Level (11-15)
    {
      id: 11,
      question: "What is the time complexity of merging two sorted arrays?",
      answer: [
        { text: "O(n)", correct: true },
        { text: "O(n^2)", correct: false },
        { text: "O(log n)", correct: false },
        { text: "O(1)", correct: false },
      ],
    },
    {
      id: 12,
      question: "Which JavaScript feature allows functions to access variables from their parent scope?",
      answer: [
        { text: "Closures", correct: true },
        { text: "Promises", correct: false },
        { text: "Hoisting", correct: false },
        { text: "Async/Await", correct: false },
      ],
    },
    {
      id: 13,
      question: "In React, which hook is used to manage local component state?",
      answer: [
        { text: "useState", correct: true },
        { text: "useEffect", correct: false },
        { text: "useRef", correct: false },
        { text: "useReducer", correct: false },
      ],
    },
    {
      id: 14,
      question: "What is the purpose of Docker in software development?",
      answer: [
        { text: "To containerize applications for consistent deployment", correct: true },
        { text: "To create virtual machines", correct: false },
        { text: "To manage source code", correct: false },
        { text: "To design UI components", correct: false },
      ],
    },
    {
      id: 15,
      question : "Which data structure is typically used in Depth-First Search (DFS)?",
      answer: [
        { text: "Stack", correct: true },
        { text: "Queue", correct: false },
        { text: "Heap", correct: false },
        { text: "Tree", correct: false },
      ],
    },
  ];

  export default data;